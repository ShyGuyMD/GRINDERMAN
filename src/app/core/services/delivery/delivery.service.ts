...
export class DeliveryService {
  ...
  private deliveryMapAddress$: BehaviorSubject<string> = new BehaviorSubject<string>('');
  private deliveryArea$: BehaviorSubject<google.maps.LatLngLiteral[]> = new BehaviorSubject<google.maps.LatLngLiteral[]>([]);
  private departamentos$: BehaviorSubject<Departamento[]> = new BehaviorSubject<Departamento[]>([]);
  ...
  constructor(private _apiService: ApiService) {
    ...
    .subscribe((departamentos: Departamento[]) => {
      ...
      departamentos.forEach(dep => {
        if(dep.name === 'MONTEVIDEO'){
          dep.cities.forEach(city => {
            if(city.localidad === 'MONTEVIDEO'){
              this.setDeliveryArea(city.area);
            }
          });
        }
      });
  ...
  public getMapAddress(): Observable<string> {
    return this.deliveryMapAddress$.asObservable();
  }

  public setMapAddress(address: string): void {
  ...
  public loadCities(): Observable<Departamento[]>  {
      ...
      map((data: CityImport[]) => {
        ...
        data.forEach((city: CityImport) => {
          ...
          if (!departamentoMap.has(city.departamento)) {
            departamentoMap.set(city.departamento, { name: city.departamento, cities: [] });
          }
          const geoJsonData = wellknown.parse(city.wkt);
          let area : google.maps.LatLngLiteral[] = [];
          if (geoJsonData && geoJsonData.type === 'MultiPolygon' && Array.isArray(geoJsonData.coordinates)) {
...
private convertCoordinates(coordinates: number[][]): google.maps.LatLngLiteral[] {
